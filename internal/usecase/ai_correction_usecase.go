...
func NewAICorrectionUseCase(aiRepo repository.AIRepository) *AICorrectionUseCase {
...
// Correct テキストを補正
func (uc *AICorrectionUseCase) Correct(text string) (*entity.AIResult, error) {
	// 入力検証
...
func (uc *AICorrectionUseCase) RecognizeImage(imageData []byte) (*entity.AIResult, error) {
	// 入力検証
	if len(imageData) == 0 {
...
func (uc *AICorrectionUseCase) RecognizeReceipt(imageData []byte) (*entity.AIResult, error) {
	// 入力検証
	if len(imageData) == 0 {
		return nil, fmt.Errorf("image data is empty")
	}
...
func (uc *AICorrectionUseCase) CategorizeReceipt(receiptInfo string) (*entity.AIResult, error) {
	// 入力検証
