...
func TestReceipt_AddItem(t *testing.T) {
	receipt := NewReceipt(uuid.NewString(), "ストア", time.Now(), 1000, 100, "食費")

	item1 := NewReceiptItem(uuid.NewString(), receipt.ID, "商品1", 2, 500)
	item2 := NewReceiptItem(uuid.NewString(), receipt.ID, "商品2", 1, 300)
...
func TestReceipt_TotalItems(t *testing.T) {
	...
	// 3個追加
	for i := 0; i < 3; i++ {
		item := NewReceiptItem(uuid.NewString(), receipt.ID, "商品", 1, 100)
...
func TestExpenseEntry_WithReceiptID(t *testing.T) {
	receiptID := uuid.NewString()
	entry := NewExpenseEntry(
		uuid.NewString(),
		time.Now(),
	...
	if entry.ReceiptID != nil {
		t.Error("ReceiptID should be nil initially")
	}

	// ReceiptIDを設定
	entry.ReceiptID = &receiptID
	if entry.ReceiptID == nil {
		t.Error("ReceiptID should not be nil")
	}
	if *entry.ReceiptID != receiptID {
		t.Errorf("ReceiptID = %v, want %v", *entry.ReceiptID, receiptID)
	}
}

func TestExpenseEntry_EmptyTags(t *testing.T) {
	entry := NewExpenseEntry(
		uuid.NewString(),
		time.Now(),
		"食費",
	...
	if len(entry.Tags) != 0 {
		t.Errorf("Tags length = %v, want 0", len(entry.Tags))
	}
}

func TestExpenseEntry_NilTags(t *testing.T) {
	entry := NewExpenseEntry(
		uuid.NewString(),
		time.Now(),
		"食費",
